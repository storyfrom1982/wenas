...
static void test_byte_order()
{
    ...
    LOGD("LINEDB", "bit shift: little to little => little number= %x swap number= %x byte= %x %x %x %x\n",
    ...
    LOGD("LINEDB", "swap byte: little to little => little number= %x swap number= %x byte= %x %x %x %x\n",
    ...
    LOGD("LINEDB", "swap byte: little to big => little number=%x byte[]= %x %x %x %x\n", little_number, swap_byte[0], swap_byte[1], swap_byte[2], swap_byte[3]);
    LOGD("LINEDB", "swap byte: little to big => little number=%x big number=%x\n", little_number, big_number);
    ...
    big_number = swap_byte[3] << 24 | swap_byte[2] << 16 | swap_byte[1] << 8 | swap_byte[0];
    LOGD("LINEDB", "bit shift: little to big => little number= %x byte[]= %x %x %x %x\n", little_number, swap_byte[0], swap_byte[1], swap_byte[2], swap_byte[3]);
    LOGD("LINEDB", "swap byte: little to big => little number=%x big number=%x\n", little_number, big_number);
    ...
    LOGD("LINEDB", "bit shift: big to byte => big number= %x byte[]= %x %x %x %x\n", big_number, swap_byte[0], swap_byte[1], swap_byte[2], swap_byte[3]);
    little_number = swap_byte[0] << 24 | swap_byte[1] << 16 | swap_byte[2] << 8 | swap_byte[3];
    LOGD("LINEDB", "bit shift: big to little => big number=%x little number=%x\n", big_number, little_number);
    LOGD("LINEDB", ">>>>------------>\n");
}
...
static void number16_to_ldb()
{
    ...
    n16 = __b2n16(&ldb);
    LOGD("LINEDB", "block head = 0x%x ldb.byte[0] >> 7 = 0x%x ldb.byte[0] & 0x80 = 0x%x\n", 
        ldb.byte[0], (uint8_t)(ldb.byte[0] >> 7), ldb.byte[0] & 0x80);

    uint32_t size = __sizeof_linedb(&ldb);
    LOGD("LINEDB", "int16_t %d to linear block %d sizeof(block)=%u\n", i16, n16, size);

    int16_t u16, iu16 = 12345;
    ldb = __n2b16(iu16);
    u16 = __b2n16(&ldb);
    size = __sizeof_linedb(&ldb);
    LOGD("LINEDB", "uint16_t %u to linear block %u sizeof(block)=%u\n", iu16, u16, size);
...
static void number32_to_ldb()
{
    ...
    uint32_t size = __sizeof_linedb(&ldb);
    n32 = __b2n32(&ldb);
    LOGD("LINEDB", "int32_t %d to linear block %d sizeof(block)=%u\n", i32, n32, size);

    uint32_t u32, iu32 = 12345;
    ldb = __n2b32(iu32);
    size = __sizeof_linedb(&ldb);
    u32 = __b2n32(&ldb);
    LOGD("LINEDB", "uint32_t %u to linear block %u sizeof(block)=%u\n", iu32, u32, size);
...
static void number64_to_ldb()
{
    ...
    uint32_t size = __sizeof_linedb(&ldb);
    n64 = __b2n64(&ldb);
    LOGD("LINEDB", "int64_t %ld to linear block %ld sizeof(block)=%u\n", i64, n64, size);

    uint64_t u64, iu64 = 12345;
    ldb = __n2b64(iu64);
    size = __sizeof_linedb(&ldb);
    u64 = __b2n64(&ldb);
    LOGD("LINEDB", "uint64_t %lu to linear block %lu sizeof(block)=%u\n", iu64, u64, size);
...
{
    LOGD("LINEDB", "BLOCK_TYPE_BOOLEAN=0x%x\n", 0x03 << 6);
    LOGD("LINEDB", "BLOCK_TYPE_FLOAT=0x%x\n", 0x02 << 6);
    LOGD("LINEDB", "BLOCK_TYPE_UNSIGNED=0x%x\n", 0x01 << 6);
    LOGD("LINEDB", "BLOCK_TYPE_INTEGER=%u\n", LINEDB_NUMBER_INTEGER);
    LOGD("LINEDB", ">>>>------------>\n");

    LOGD("LINEDB", "BLOCK_TYPE_8BIT | BLOCK_TYPE_INTEGER=0x%x\n", (LINEDB_TYPE_8BIT | LINEDB_NUMBER_INTEGER));
    LOGD("LINEDB", "BLOCK_TYPE_8BIT | BLOCK_TYPE_UNSIGNED=0x%x\n", (LINEDB_TYPE_8BIT | LINEDB_NUMBER_UNSIGNED));
    LOGD("LINEDB", "BLOCK_TYPE_8BIT | BLOCK_TYPE_FLOAT=0x%x\n", (LINEDB_TYPE_8BIT | LINEDB_NUMBER_FLOAT));
    LOGD("LINEDB", "BLOCK_TYPE_8BIT | BLOCK_TYPE_BOOLEAN=0x%x\n", (LINEDB_TYPE_8BIT | LINEDB_NUMBER_BOOLEAN));
    LOGD("LINEDB", ">>>>------------>\n");

    LOGD("LINEDB", "BLOCK_TYPE_8BIT | BLOCK_TYPE_INTEGER=0x%x\n", (LINEDB_TYPE_8BIT | LINEDB_NUMBER_INTEGER) >> 6);
    LOGD("LINEDB", "BLOCK_TYPE_16BIT | BLOCK_TYPE_UNSIGNED=0x%x\n", (LINEDB_TYPE_16BIT | LINEDB_NUMBER_UNSIGNED) >> 6);
    LOGD("LINEDB", "BLOCK_TYPE_32BIT | BLOCK_TYPE_FLOAT=0x%x\n", (LINEDB_TYPE_32BIT | LINEDB_NUMBER_FLOAT) >> 6);
    LOGD("LINEDB", "BLOCK_TYPE_64BIT | BLOCK_TYPE_BOOLEAN=0x%x\n", (LINEDB_TYPE_64BIT | LINEDB_NUMBER_BOOLEAN) >> 6);
    LOGD("LINEDB", ">>>>------------>\n");
}
