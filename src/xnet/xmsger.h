...
typedef struct xmsgercb {
    ...
    void (*on_msg_from_peer)(struct xmsgercb*, xchannel_ptr channel, xlmsg_ptr xkv);
    void (*on_msg_to_peer)(struct xmsgercb*, xchannel_ptr channel, xlmsg_ptr xkv);
...
extern bool xmsger_send_message(xmsger_ptr msger, xchannel_ptr channel, xlmsg_ptr msg);
extern bool xmsger_connect(xmsger_ptr msger, const char *ip, uint16_t port, void *ctx, xlmsg_ptr firstmsg);
