...
struct xchannel {
    ...
    xmsg_ptr send_list_head, confirm_list_head;
    xmsg_ptr *msglist_tail, *streamlist_tail;
    xmsg_ptr streams[3];
...
static inline void xchannel_free(xchannel_ptr channel)
{
    ...
    for (int i = 0; i < 3; ++i){
        if (channel->streams[i] != NULL){
            // TODO 会导致 on_msg_from_peer 中释放内存崩溃
            free(channel->streams[i]);
        }
    }
...
static inline void xchannel_serial_msg(xchannel_ptr channel)
{
    ...
    if (channel->connected && msg != NULL && __serialbuf_writable(channel->sendbuf) > 0){
        ...
        if (msg->sendpos == msg->lkv.wpos){
            if (channel->confirm_list_head == NULL){
                channel->confirm_list_head = channel->send_list_head;
            }
            // 更新当前消息
            channel->send_list_head = channel->send_list_head->next;
            msg = channel->send_list_head;
...
static inline void xchannel_send_msg(xchannel_ptr channel, xmsg_ptr msg)
{
    ...
    __xlogd("xchannel_send_msg 1\n");
    __atom_add(channel->msger->len, msg->lkv.wpos);
    __xlogd("xchannel_send_msg 2\n");
    channel->msger->msglistlen++;

    // 判断是否有正在发送的消息
    if (channel->send_list_head == NULL){ // 当前没有消息在发送
        // 第一个消息的 next 将成为队尾标志，所以 next 必须设置成 NULL
        msg->next = NULL;
        // 设置新消息为当前正在发送的消息
        channel->send_list_head = msg;

        if (msg->streamid == 0){ // 新成员是消息
            // 加入消息队列
        ...
        if (msg->streamid == 0){ // 新成员是消息
            ...
            if (channel->send_list_head->streamid == 0){ // 当前正在发送的是消息队列
                // 将新成员的 next 指向消息队列尾部
                ...
                *(channel->msglist_tail) = msg;
                ...
                msg->next = channel->send_list_head;
                // 设置新消息成员为队首
                channel->send_list_head = msg;
                // 将消息队列的尾部指针指向新成员
                channel->msglist_tail = &(msg->next);
            ...
            if (channel->send_list_head->streamid == 0){ // 如果当前正在发送的是消息队列
    ...
    if (__serialbuf_recvable(channel->sendbuf) > 0 && ((uint8_t)(rpack->head.ack - channel->sendbuf->rpos) <= (uint8_t)(channel->sendbuf->spos - channel->sendbuf->rpos))){
        ...
        while (channel->sendbuf->rpos != rpack->head.acks) {
            ...
            if (pack->timestamp != 0){
                ...
                // 判断是否有未发送的消息
                if (channel->send_list_head == NULL){
            ...
            if (pack->head.type == XMSG_PACK_MSG){
                ...
                if (pack->msg->recvpos == pack->msg->lkv.wpos){
                    // 更新待确认队列
                    channel->confirm_list_head = channel->confirm_list_head->next;
...
static void* main_loop(void *ptr)
{
    ...
    while (__is_true(msger->running))
    {
        ...
        while (__xapi->udp_recvfrom(msger->sock, &addr, &rpack->head, PACK_ONLINE_SIZE) == (rpack->head.len + PACK_HEAD_SIZE)){
            ...
            __xlogd("main_loop (IP:%X)(KEY:%u) >>>>>---------------------------------> RECV: TYPE(%u) FLAG[%u:%u:%u] SN: %u\n", 
                    *(uint64_t*)(&addr.port), rpack->head.key, rpack->head.type, rpack->head.flag, rpack->head.ack, rpack->head.acks, rpack->head.sn);
            ...
            if (channel){
                ...
                if ((rpack->head.key ^ channel->local_key) == XMSG_VAL){
                    ...
                    __xlogd("main_loop key ok\n");

                    if (channel->connected && rpack->head.type == XMSG_PACK_MSG) {
                        ...
                        __xlogd("main_loop XMSG_PACK_PONG 1\n");
                        // 主动端收到了 PONG
                        if (!channel->connected){
                            __xlogd("main_loop XMSG_PACK_PONG 2\n");
                            // 更新连接状态
                            channel->connected = true;
                            // 取出同步参数
                        ...
                        __xlogd("main_loop XMSG_PACK_PONG 3\n");
                        // 更新接收缓冲区和 ACK
                        xchannel_recv_pack(channel, &rpack);
                        // 回复 ACK
                        if (channel->ack.flag != 0){
                            __xlogd("main_loop XMSG_PACK_PONG 4\n");
                            xchannel_send_ack(channel);
                        }

                        if (channel->send_list_head){
                            __xlogd("main_loop XMSG_PACK_PONG 5\n");
                            xchannel_serial_msg(channel);
                        }
                        __xlogd("main_loop XMSG_PACK_PONG 6\n");
                    ...
                    __xlogd("main_loop key XMSG_KEY\n");

                    if (rpack->head.type == XMSG_PACK_PING){

                        // 无法实现无感知自动重连，因为这里无法区分是重连发送的 PING 还是超时重传的 PING
                        __xlogd("main_loop RECV XMSG_PACK_PING\n");
                        if (rpack->head.resend > 0){
                            xchannel_recv_pack(channel, &rpack);
                            ...
                            uint8_t serial_number = *((uint8_t*)(rpack->body + 2));
                            // 创建连接
                            xchannel_ptr ch = xchannel_create(msger, serial_range);
                            __xbreak(ch == NULL);
                            ch->addr = addr;
                            // 同步序列号
                            ch->recvbuf->rpos = ch->recvbuf->spos = ch->recvbuf->wpos = serial_number;
                            // 设置校验码
                            ch->remote_key = remote_key;
                            __xlogd("xmsger_loop >>>>-------------> RECV PING: SN(%u) REMOTE KEY(%u) LOCAL KEY(%u) ts(%lu)\n", serial_number, ch->remote_key, ch->local_key, ch->timestamp);
                            // 生成 ack 的校验码
                            ch->ack.key = (XMSG_VAL ^ ch->remote_key);
                            // 更新接收缓冲区和 ACK
                            xchannel_recv_pack(ch, &rpack);
                            // 生成 PONG
                            xchannel_serial_pack(ch, XMSG_PACK_PONG);
                            // 发送 PONG 和更新的 ACK
                            ...
                            // 建立索引
                            xtree_save(msger->peers, &addr.port, 6, ch);
                            ch->connected = true;
                            msger->callback->on_channel_from_peer(msger->callback, ch);
                        }

                    }else if (rpack->head.type == XMSG_PACK_ACK && rpack->head.flag == XMSG_PACK_BYE){
                        msger->callback->on_channel_break(msger->callback, channel);
